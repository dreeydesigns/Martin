      ...
      <main className={`w-full max-w-md md:max-w-4xl lg:max-w-5xl mx-auto sm:shadow-2xl sm:border sm:rounded-2xl overflow-hidden relative transition-colors duration-500 ${t.cardBg} ${t.border}`}>
        ...
        >
          ...
          <form onSubmit={handleSendMessage} className="space-y-4">
            {formError && <p className="text-red-500 text-xs text-center font-medium mb-2">{formError}</p>}
            <div>
              <input 
                type="text" 
                value={formName}
...
function ActionButton({ href, icon, label, t, isDarkMode, onClick }: { href: string, icon: ReactNode, label: string, t: any, isDarkMode: boolean, onClick?: () => void }) {
  return (
    <motion.a 
      href={href}
...
function SocialLink({ href, icon, t, isDarkMode, onClick }: { href: string, icon: ReactNode, t: any, isDarkMode: boolean, onClick?: () => void }) {
  return (
    <motion.a 
      href={href}
      target="_blank"
